...
### Transition setting
BILLIO et al. (2016) implements the covariate method from Kaufmann (2014): K-state switching models with time-varying transition distributions – Does credit growth signal stronger effects of variables on inflation?.
...
$$
P(S_{t} = k | Z_{t}, \gamma) = \xi_{kt} = \frac{exp(Z_{t} \gamma^{z}_{k} + \gamma_{k})}
{\sum^{K}_{j=1}exp(Z_{t} \gamma^{z}_{j} + \gamma_{j})}
$$
