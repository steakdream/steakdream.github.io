...
$$
\unerline{\psi} = 0, \; \tau_{i} = I, \underline{\tau} = 10 I
$$
...
$$
\underline{\lambda_0} = [0.5 -0.3 0.2 0.4]

\underline{\Sigma_{0}} = diag(0.1, K*K*P)

\underline{\Sigma_{i, 0}} = replicate(N, diag(0.1, K*K*P), simplify = FALSE)
$$

$$
\underline{\lambda_{m = 1}} = rep(-1, K), \; \underline{\lambda_{m = 2}} = rep(1, K)

\underline{\Sigma_{m}} = diag(0.1, K)

\underline{\Sigma_{i, m}} = diag(0.1, K)

\underline{\Sigma_{i, 0}} = replicate(N, diag(0.1, K), simplify = FALSE)
$$
...
\underline{\Ups} = diag(0.1, 2)

\underline{\Ups_{i}} = replicate(N, diag(0.1, K), simplify = FALSE)
$$
