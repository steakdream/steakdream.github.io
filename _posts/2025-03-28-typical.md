...
, where $Z_t$ is the covariate. For identification purpose, the reference state is the last regime, M, which means that the coefficients $\gamma^{z}_{lm}$ \text{and} $\gamma_{lm}$ with m=M are zeros.
